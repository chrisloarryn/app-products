...
async function getProducts(req, res, next) {
    try {
        ...
        const stations = await dataFetcher(`https://my-json-server.typicode.com/jirann/FAKEJSON/user/1/items`);
        const users = await dataFetcher(`https://my-json-server.typicode.com/jirann/FAKEJSON/profile`)
        const data = users.map(user => {
            return {
                user,
                stations: stations.filter(station => parseInt(user.id) === station.user_id)
            }
        });
        const myDataFormat = {
            users,
